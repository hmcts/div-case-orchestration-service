...
@Rule(order = 5)
@Data
public class PetitionerEmail {

    private static final String BLANK_SPACE = " ";
    private static final String ACTUAL_DATA = "Actual data is: %s";
    private static final String ERROR_MESSAGE = "petitionerEmail can not be null or empty.";
    ...
    @When
    public boolean when() {
        return !Optional.ofNullable(divorceSession.getPetitionerEmail()).isPresent();
    }
    ...
    public void then() {
        result.add(String.join(
            ...
            ERROR_MESSAGE,
            String.format(ACTUAL_DATA, divorceSession.getPetitionerEmail())
        ));
    }
}