...
public class DocumentDataExtractor {
    ...
    public static GeneratedDocumentInfo.GeneratedDocumentInfoBuilder getDocumentInformPartiallyPopulated(
        Map<String, Object> caseData, String targetDocumentType
    ) {
        Map<String, Object> data = getGeneratedDocumentsFromCaseData(caseData)
            .stream()
            .filter(element -> DocumentDataExtractor.isSpecifiedDocument(element, targetDocumentType))
            .map(element -> getNotEmptyField(element, CaseDataKeys.COMPLEX_TYPE_COLLECTION_ELEMENT))
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException(String.format("% should be found in caseData!", targetDocumentType)));
    ...
    public static boolean isSpecifiedDocument(Map<String, Object> collectionMember, String targetDocumentType) {
        Map<String, Object> document = getNotEmptyField(collectionMember, CaseDataKeys.COMPLEX_TYPE_COLLECTION_ELEMENT);

        return targetDocumentType.equals(document.get(CaseDataKeys.DOCUMENT_TYPE));
    }
