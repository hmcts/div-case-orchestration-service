...
@Component
@Slf4j
public class SendCoRespondentCertificateOfEntitlementNotificationEmailTask implements Task<Map<String, Object>> {

    private static final String EMAIL_DESCRIPTION = "Co-Respondent Notification - Certificate of Entitlement";
    ...
    public Map<String, Object> execute(TaskContext context, Map<String, Object> caseDataPayload) throws TaskException {
        log.info("Executing task to notify co-respondent about certificate of entitlement. Case id: {}",
            (String) context.getTransientObject(CASE_ID_JSON_KEY));

        String coRespEmail = (String) caseDataPayload.get(CO_RESP_EMAIL_ADDRESS);
        String reasonForDivorce = (String) caseDataPayload.get(D_8_REASON_FOR_DIVORCE);

        if (StringUtils.isNotBlank(coRespEmail) && ADULTERY.equalsIgnoreCase(reasonForDivorce)) {
            ...
            String coRespFirstName = getMandatoryPropertyValueAsString(caseDataPayload, D8_REASON_FOR_DIVORCE_ADULTERY_3RD_PARTY_FNAME);
            String coRespLastName =  getMandatoryPropertyValueAsString(caseDataPayload, D8_REASON_FOR_DIVORCE_ADULTERY_3RD_PARTY_LNAME);
            ...
            try {
                templateParameters.put(NOTIFICATION_ADDRESSEE_FIRST_NAME_KEY, coRespFirstName);
                templateParameters.put(NOTIFICATION_ADDRESSEE_LAST_NAME_KEY, coRespLastName);
                templateParameters.put(NOTIFICATION_EMAIL, coRespEmail);
                ...
                template = EmailTemplateNames.CO_RESPONDENT_CERTIFICATE_OF_ENTITLEMENT_NOTIFICATION;
                emailToBeSentTo = coRespEmail;
