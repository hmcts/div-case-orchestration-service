...
public class BulkCaseServiceImpl implements BulkCaseService {
    ...
    private final UpdateCaseWithinBulkWorkflow updateCaseWithinBulkWorkflow;
    ...
    public void handleBulkCaseCreateEvent(BulkCaseCreateEvent event) throws WorkflowException {
        ...
        log.info("Completed bulk case creation with bulk cased Id:{} in:{} millis", bulkCaseId, endTime - startTime);
    }
    ...
    public void handleBulkCaseUpdateCourtHearingEvent(BulkCaseUpdateCourtHearingEvent event) throws WorkflowException {
        ...
        updateBulkCaseWorkflow.run(emptyMap(), context.getTransientObject(AUTH_TOKEN_JSON_KEY), bulkCaseId, LISTED_EVENT);
    ...
    @Override
    @EventListener
    public void handleBulkCaseRemovePronouncementDetailsEvent(BulkCaseRemovePronouncementDetailsEvent event) {
        final long startTime = Instant.now().toEpochMilli();
        ...
        for (String caseId : casesToUnlink) {
            try {
                // Triggering event does not require payload
                updateCaseWorkflow.run(
                        emptyMap(),
                        context.getTransientObject(AUTH_TOKEN_JSON_KEY),
                        caseId,
                        REMOVE_FROM_BULK_CASE_LISTED_EVENT
                );
            } catch (WorkflowException e) {
                log.error("Error removing bulk case pronouncement details with bulkCaseId: {} and caseId {}", bulkCaseId, caseId);
            }
        }
