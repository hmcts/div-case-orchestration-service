...
@Slf4j
@Component
public class CourtServiceValidationTask implements Task<Map<String, Object>> {

    @Override
    public Map<String, Object> execute(TaskContext context, Map<String, Object> payload) throws TaskException {
        String solServiceMethod = getMandatoryPropertyValueAsString(payload, SOL_SERVICE_METHOD_CCD_FIELD);
        if (!COURT_SERVICE_VALUE.equals(solServiceMethod)) {
            final String caseId = context.getTransientObject(CASE_ID_JSON_KEY);
            log.error(
                    "Unexpected service method for migration to personal service - Case ID: {}", caseId
            );
            throw new TaskException(
                    "This event can only be used with for a case with Court Service as the service method"
            );
        }
        return payload;
    }
