...
public class SubmitDnCase implements Task<Map<String, Object>> {
    ...
    private String getDnEventId(final CaseDetails currentCaseDetails, Map<String, Object> caseData, TaskContext context) {
        ...
        if (AWAITING_CLARIFICATION.equalsIgnoreCase(caseState)) {
            return evaluateEventId(context, caseData, DN_RECEIVED_CLARIFICATION, BO_WELSH_SUBMIT_DN_CLARIFICATION_EVENT_ID);
        } else if (AOS_COMPLETED.equalsIgnoreCase(caseState)) {
            return evaluateEventId(context, caseData, DN_RECEIVED_AOS_COMPLETE, BO_WELSH_DN_RECEIVED_AOS_COMPLETED_EVENT_ID);
        } else {
            return evaluateEventId(context, caseData, DN_RECEIVED, BO_WELSH_DN_RECEIVED_EVENT_ID);
        }
    }
