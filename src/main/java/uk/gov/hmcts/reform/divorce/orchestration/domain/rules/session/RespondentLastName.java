...
@Rule(order = 12)
@Data
public class RespondentLastName {

    private static final String BLANK_SPACE = " ";
    private static final String ACTUAL_DATA = "Actual data is: %s";
    private static final String ERROR_MESSAGE = "respondentLastName can not be null or empty.";
    ...
    @When
    public boolean when() {
        return !Optional.ofNullable(divorceSession.getRespondentLastName()).isPresent();
    }
    ...
    public void then() {
        result.add(String.join(
            ...
            ERROR_MESSAGE,
            String.format(ACTUAL_DATA, divorceSession.getRespondentLastName())
        ));
    }
}