...
@Rule(order = 22)
@Data
public class ClaimsCosts {

    private static final String BLANK_SPACE = " ";
    private static final String ACTUAL_DATA = "Actual data is: %s";
    private static final String ERROR_MESSAGE = "claimsCosts can not be null or empty.";
    ...
    @When
    public boolean when() {
        return !Optional.ofNullable(divorceSession.getClaimsCosts()).isPresent();
    }
    ...
    public void then() {
        result.add(String.join(
            ...
            ERROR_MESSAGE,
            String.format(ACTUAL_DATA, divorceSession.getClaimsCosts())
        ));
    }
}