package uk.gov.hmcts.reform.divorce.orchestration.workflows;

import org.apache.commons.lang3.tuple.ImmutablePair;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;
import uk.gov.hmcts.reform.divorce.orchestration.domain.model.ccd.CaseDetails;
import uk.gov.hmcts.reform.divorce.orchestration.framework.workflow.DefaultWorkflow;
import uk.gov.hmcts.reform.divorce.orchestration.framework.workflow.WorkflowException;
import uk.gov.hmcts.reform.divorce.orchestration.framework.workflow.task.Task;
import uk.gov.hmcts.reform.divorce.orchestration.tasks.CaseFormatterAddDocuments;
import uk.gov.hmcts.reform.divorce.orchestration.tasks.CertificateOfEntitlementGenerator;

import java.util.Map;

import static uk.gov.hmcts.reform.divorce.orchestration.domain.model.OrchestrationConstants.AUTH_TOKEN_JSON_KEY;
import static uk.gov.hmcts.reform.divorce.orchestration.domain.model.OrchestrationConstants.CASE_DETAILS_JSON_KEY;

@Component
public class CertificateOfEntitlementGeneratorWorkflow extends DefaultWorkflow<Map<String, Object>> {

    private final CertificateOfEntitlementGenerator certificateOfEntitlementGenerator;
    private final CaseFormatterAddDocuments caseFormatterAddDocuments;

    @Autowired
    public CertificateOfEntitlementGeneratorWorkflow(CertificateOfEntitlementGenerator certificateOfEntitlementGenerator,
                                                     CaseFormatterAddDocuments caseFormatterAddDocuments) {
        this.certificateOfEntitlementGenerator = certificateOfEntitlementGenerator;
        this.caseFormatterAddDocuments = caseFormatterAddDocuments;
    }

    public Map<String, Object> run(CaseDetails caseDetails, String authorizationToken) throws WorkflowException {
        return execute(
                new Task[] {
                    certificateOfEntitlementGenerator,
                    caseFormatterAddDocuments
                },
                caseDetails.getCaseData(),
                ImmutablePair.of(AUTH_TOKEN_JSON_KEY, authorizationToken),
                ImmutablePair.of(CASE_DETAILS_JSON_KEY, caseDetails)
        );
    }
}