...
public class DataExtractionToFamilyManTest extends MockedFunctionalTest {
    ...
    public void shouldEmailCsvFileWithCase_ForDecreeAbsoluteIssued() throws Exception {
        ...
        await().untilAsserted(() -> {
            ...
            verify(mockEmailClient, times(1)).sendEmailWithAttachment(any(), any(), any());

            verify(mockEmailClient).sendEmailWithAttachment(eq("da-extraction@divorce.gov.uk"),
                eq(expectedAttachmentName),
