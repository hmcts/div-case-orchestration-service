...
* `Login to Azure` -> Go to `Key vaults` -> `div-aat` -> `Secrets` (under Settings left menu). Click on what you are looking -> Click `CURRENT VERSION` -> Click on the UUID that to reveal more info, click on Show Secret Value. 
